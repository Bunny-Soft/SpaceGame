...
GameOverWindow::GameOverWindow() {
    this -> set_title("Game Over");
    this -> set_border_width(5);

    if (ship_type == 1) {
        image.set("background_1_moving_gameover.gif");
        grid.attach(image, 0, 0, 4, 4);
    } else if (ship_type == 2) {
        image.set("background_1_moving_gameover_2.gif");
        grid.attach(image, 0, 0, 4, 4);
    } else if (ship_type == 3) {
        image.set("background_1_moving_gameover_3.gif");
        grid.attach(image, 0, 0, 4, 4);
    }

    exit2.add_label("Exit");
    exit2.signal_pressed().connect(sigc::mem_fun( * this, &GameOverWindow::Exit));
    grid.attach(exit2, 0, 4, 4, 2);
